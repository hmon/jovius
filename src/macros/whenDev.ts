...
export function whenDev(config: UserConfig, t: MacroParams["babel"]["types"], references?: Babel.NodePath[]) {
  const { env } = config;
  ...
  references.forEach(referencePath => {
    ...
    if (args.length < 1 || args.length > 2) {
      throw new Error('whenDev expects 1 or 2 arguments');
    }

    // check if the first argument is a function
    if (isFunction(t, args[0])) {
      if (isAssignment) {
        throw new Error('whenDev cannot be assigned to a variable when the first argument is a function');
      }
